...
def StockSearch(event=None):
    ...
    if itemno == "" or manufact == "Select Manufacturer" or reqty == 0:
        tkMessageBox.showinfo("Smart Inventory System", "All fields should be mandatory!!")
        return
...
def SearchForm():
    ...
    body = "Hi All,\n Please find the attached link of different manufacturer to procure below Item: \n Part NO. -  ", itemno, "\n Required Quntity - ", reqty, "\n \n Note: Please do not reply to this message"
...
def Update():
    ...
    for i in range(len(stockdetails)):
        if partno == 'Select Part Number' or manufacturer == "" or description == "" or quantity == 0 or price == 0 or bulk == 0 or remarks == "":
            tkMessageBox.showinfo("Update Inventory Stock", "All fields should be mandatory!!")
            return
...
def AddNew():
    ...
    if Part_Number.get() == "" or Part_Manufecturer.get() == "" or Part_Description.get() == "" or Total_Qty.get() == 0 or Calibration_EndDate.get() == "" or Price_Per_Piece.get() == 0 or Price_In_Bulk.get() == 0 or Remarks.get() == "":
        tkMessageBox.showinfo('Add Inventory Data', "All fields should be mandatory!")
        return
...
def Bom():
    ...
    if not tree.selection():
        tkMessageBox.showinfo('Smart Inventory System', "Please select the row!")
    else:
        ...
        if os.path.isfile(path):
            ...
            writer.save()
            tkMessageBox.showinfo("BOM File", "Please navigate to Automated_Bom folder from current directory to find the generated BOM!")
            Reset()
